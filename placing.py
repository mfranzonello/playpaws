...
from pandas import DataFrame, Series, isnull, to_datetime, Timestamp
...
from display.storage import Boxer, GClouder, Closet

class Placer(Streamable): 
    marker_sizing = 50
    ...
    def __init__(self, database):
        ...
        self.boxer = Boxer()
        self.canvas = Canvas(database)

        self.closet = Closet(None, GClouder())
        self.database = database

        self.library.add_emoji(*self.database.get_emojis())

        self.god_player = self.database.get_god_id()
        self.plot_names = self.database.get_plot_names()
        self.plot_combinations = self.database.get_plot_combinations()
        self.plot_updates = self.plot_combinations.copy()
    ...
    # generate charts
    def generate_charts(self, league_round_ids={}, player_image_ids=[]):
        ''' create charts '''

        dfs = self.get_dfs() ## define new function

        league_specific = ['title', 'hoarding', 'features', 'tags', 'pie', 'top_songs_summary', 'playlists']
        player_specific = ['viewer', 'caption']
        league_player_specific = ['viewer_league', 'caption_league', 'title', 'mappings', 'boards', 'scores']
        league_round_specific = ['top_songs_round']
        league_round_player_specific = []
        specifics = league_specific + player_specific + league_player_specific + league_round_specific + league_round_player_specific
        self.plot_updates.loc[:, specifics] = False

        for player_id in player_image_ids:
            ## player specific functions
            key = {'player_id': player_id}
            self.update_chart(self.plot_viewer, key, 'viewer', player_id=player_id, ...)
            self.update_chart(self.plot_caption, key, 'caption', player_id=player_id, ...)
                
            query = self.plot_combinations.query('player_id == @player_id')
            player_ids = query['league_id'].unique()
            for league_id in league_ids:
                ## league and player specific functions -> need to mark as already complete, so don't double up
                key = {'player_id': player_id, 'league_id': league_id}
                self.update_chart(self.plot_viewer, key, 'viewer_league', player_id=player_id, league_id=league_id, ...) ## define new function
                self.update_chart(self.plot_mappings, key,'mappings', league_id=league_id, ...)
                self.update_chart(self.plot_boards, key, 'boards', league_id=league_id, ...)
                self.update_chart(self.plot_scores, key, 'scores', league_id=league_id, ...)
                
        for league_id in league_ids:
            ## league specific functions
            key = {'league_id': league_id}
            self.update_chart(self.plot_hoarding, key, 'hoarding', league_id=league_id, ...)
            self.update_chart(self.plot_features, key, 'features', league_id=league_id, ...)
            self.update_chart(self.plot_tags, key, 'tags', league_id=league_id, ...)
            self.update_chart(self.plot_pie, key, 'pie', league_id=league_id, ...)
            self.update_chart(self.plot_top_songs_summary, key, 'top_songs_summary', league_id=league_id, ...)
            self.update_chart(self.plot_playlists, key, 'playlists', league_id=league_id, ...)
            
            query = self.plot_combinations.query('league_id == @league_id')
            player_ids = query['player_id'].unique()
            for player_id in player_ids:
                ## league and player specific functions
                key = {'league_id': league_id, 'player_id': player_id}
                self.update_chart(self.plot_title, key, 'title', league_id=league_id, player_id, ...)
                self.update_chart(self.plot_caption_league, key, 'caption_league', league_id=league_id, player_id=player_id, ...)
                self.update_chart(self.plot_viewer, key, 'viewer_league', league_id=league_id, player_id=player_id ...) ## define new function
                self.update_chart(self.plot_mappings, key, 'mappings', league_id=league_id, ...)
                self.update_chart(self.plot_boards, key, 'boards', league_id=league_id, ...)
                self.update_chart(self.plot_scores, key, 'scores', league_id=league_id, ...)

            round_ids = query['round_id'].unique()
            for round_id in round_ids:
                ## league and round specific functions
                if (league_id in league_round_ids) and (round_id in league_round_ids[league_id]):
                    key = {'league_id': league_id, 'round_id': round_id}
                    self.update_chart(self.plot_top_songs_round, key, 'top_songs_round', league_id=league_id, round_id=round_id, ...)

                query = query.query('round_id == @round_id')
                player_ids = query['player_id'].unique()
                for player_id in player_ids:
                    ## league, round and player specific functions
                    key = {'league_id': league_id, 'round_id': round_id, 'player_id': round_id}
                    
    def update_chart(self, func, update_col, key, **kwargs):
        query = self.plot_updates.query(' & '.join(f'{k} == {key[k]}' for k in key if key[k])) 
        if not query[update_col]:
            func(**kwargs)
            self.plot_update.loc[query.index, update_col] = True


    # name conversion
    def get_name(self, name_id, table=None, league_id=None, clean=False, full=False, display=False, feel=False, placeholder=None):
        ''' get player, league, round or competition name '''
        if isinstance(name_id, (list, ndarray, Index, Series)):
            ...
            if (not name_id) or (isinstance(name_id, str) and ((len(name_id) == 0) or (name_id == placeholder))):
                ...
                q = 'name_id == @name_id' + (' & league_id = @league_id' if league_id else '')
                query = self.plot_names[table].query(q)
                if not len(query):
                    name = name_id

                else:
                    name = query[f'{table}_name'].iloc[0]
    ...
    def get_round_title(self, round_id, league_id, clean=False):
        ''' get round title '''
        return self.get_name(round_id, league_id=league_id, table='round', clean=clean)

    def get_competition_title(self, competition_id, league_id):
        ''' get competition title '''
        return self.get_name(competition_id, league_id=league_id, table='competition')

    # common use functions
    def god_mode(self, player_id):
        ''' determine if view_player is God '''
        return player_id == self.god_player
    ...
    def rotate_labels(self, n_rounds):
        ''' turn labels by 0, 45 or 90 degress depending on number of rounds '''
        if n_rounds < 5:
            rotation = 0
        elif n_rounds < 10:
            rotation = 45
    ...
    def split_labels(self, labels, n_rounds):
        ''' split long labels into multiline '''
        if n_rounds >= 10:
    ...
    def translate(self, x:float, y:float, theta:float, rotate:float, shift_distance:float=0):
        ''' return coordiantes based on rotation and shifting '''
    ...
    def plot_viewer(self, player_id, league_id=None, battles_df, boards_competition_df):
        ''' generate image for a player '''
        plot_key = self.closet.get_key('viewer_img', league_id=league_id, player_id=player_id)
        stored, ok = self.closet.get_items(plot_key)

        if ok:
            # look for a stored player image
            image = stored
            ...
            if league_id:
                # put badges on
                queries = {'league': battles_df.query('player_id == @player_id'),
                           'competition': boards_competition_df.query('player_id == @player_id'),
                           }
                badges = {query: query['place'].iloc[0] for query in queries if len(query)}
            
                medal_metals = ['gold', 'silver', 'bronze', 'gunmetal_grey']
                for b, pct, position, label in zip([badges.get('league'), badges.get('competition')], [0.4, 0.25], ['LR', 'UL'], ['OVERALL', 'COMPETITION']):
            ...
            self.closet.store_items(plot_key, image)

    def place_badge(self, image, badge, medal_metals, pct, position, label=None):
        ''' put league or competition badge on an image '''
    ...
    def plot_caption(self, player_id, league_id=None, relations_df=None,
                     creators_winners_df, awards_round_df, stats_df=None,
                     #wins_df=None, awards_df=None, 
                     competition_title=None, badge2=None, n_players=None, competition_wins=None,
                     title=None, tab=None):
        caption_key = self.closet.get_key('caption', player_id=player_id, league_id=league_id)
        stored, ok = self.closet.get_items(caption_key)

        if ok:
            # look for stored parameters
            parameters = stored
            ...
            if self.god_mode(player_id):
                ...
                if (relations_df is not None) and len(relations_df):
                    relations_s = relations_df.set_index('player_id').loc[player_id]
                    parameters.update({k: self.get_player_name(relations_s[f'{k}_id']) for k in keys['relations'] if f'{k}_id' in relations_s})

                if (awards_round_df is not None) and len(awards_df):
                    awards_s = awards_round_df.set_index('player_id').loc[self.player_id]
                    parameters.update({k: awards_s[k] for k in keys['awards'] if k in awards_s})

                if (stats_df is not None) and len(stats_df):
                    stats_s = stats_df.set_index('player_id').loc[self.player_id]
                    parameters.update({k: stats_s[k] for k in keys['stats'] if k in stats_s})

                if (wins_df is not None) and len(wins_df):
                    wins_s = creators_winners_df.explode('winner_ids').set_index('winner_ids').loc[player_id]
                    parameters['wins'] = self.get_round_title(wins_s['round_id'], league_id)
                ...
                parameters['competition_wins'] = self.get_competition_title(competition_wins, league_id)

            self.closet.store_items(caption_key, parameters)


    def plot_title(self, league_id, player_id, creator_id):
        caption_key = self.closet.get_key('title', player_id=player_id, league_id=league_id)
        stored, ok = self.closet.get_items(caption_key)
        ...
        if ok:
            ...
            viewer = self.get_player_name(player_name)
    ...
    def plot_boards(self, league_id, boards_df, creators_winners_df, competitions_df,
        ...
        if ok:
            ...
            if ok:
                ...
                ax.set_xticks(xs)
                ax.set_xticklabels(self.split_labels(self.get_round_title(round_ids, league_id, clean=True), n_rounds),
                ...
                self.closet.store_items(plot_key_2, (ax, xs, lowest_rank, ties_df, icon_scale, maxes))     
            ...
            parameters = {m: self.get_round_title(maxes[m], league_id) for m in ['round_titles']}
            parameters.update({m: self.get_player_name(maxes[m]) for m in ['choosers', 'winners']})

            self.closet.store_items(plot_key, (ax, parameters), cloud=False)
    ...
    def place_board_player(self, ax, xs, player_id, boards_df, lowest_rank, ties_df, icon_scale,
        ...
        # plot finishers
        if highlight: ##player_id == self.view_player:
        ...
        ax.plot(xs, ys, marker='.', color=color, zorder=1 if highlight else 0, linewidth=linewidth) ##player_id==self.view_player
        ...
        for x, y, d, t in zip(xs, ys, ds, ties):
            ...
            if y > 0:
                ...
                image, _ = self.place_image(ax, x_plot, y_plot, player_id=player_id, size=size, flipped=True,
                                            zorder=3 if highlight else 2, border_color=border_color) ##player_id==self.view_player
    ...
    def plot_scores(self, league_id, rankings_df, awards_league_df, title=None, tab=None):
        ...
        if ok:
            ...
            if ok:
                ...
                self.closet.store_items(plot_key_2, (ax, player_ids, x_min, x_max, maxes))
    ...
    def plot_features(self, league_id, features_df, title=None, tab=None):
        ...
        if ok:
            ...
            features_df.loc[:, 'round'] = features_df['round_id'].apply(lambda x: self.get_round_title(x, league_id, clean=True))
    ...
    def plot_hoarding(self, league_id, awards_round_df, awards_league_df, title=None, tab=None):
        ...
        if False: #ok:
            ...
            ax.set_xticklabels(self.get_round_title(round_ids, league_id, clean=True))
            ax.legend()
...
class Plotter(Streamable): 
    ...
    def __init__(self, database, streamer):
        ...
        self.closet = Closet(streamer, GClouder())
        self.database = database
        self.placer = Placer(database)
        ...
        self.plot_counts = len((self.plot_viewer,
                                self.plot_caption,
                                self.plot_title,
                                self.plot_mappings,
                                self.plot_boards,
    ...
    def plot_results(self):
        ...
        self.view_player = self.streamer.player_box.selectbox('Who are you?',
                                                              ...
                                                              format_func=lambda x: self.placer.get_player_name(x, full=True))
        ...
        if self.view_player != self.blank_player:           
            ...
            if len(viewable_league_ids) == 1:
                ...
                league_id = self.streamer.selectbox.selectbox('Pick a league to view',
                                                                 ...
                                                                 format_func=lambda x: self.placer.get_league_title(x, feel=True))
            else:
            ...
            if league_id == self.blank_league:
                ...
                if self.placer.god_mode(self.view_player):
                    # plot the viewer image
                    self.plot_try(self.plot_viewer, league_id=league_id, player_id=player_id)

                    # plot the league stats
                    self.plot_try(self.plot_caption, league_id=league_id, player_id=player_id, title='League Stats')

                # plot the viewer stats
                else:
                    self.streamer.tab(player_tab, caption=True)
                    self.plot_try(self.plot_caption, league_id=league_id, player_id=self.view_player, tab=player_tab)

                
                # plot results title
                self.plot_try(self.plot_title, #exception=None,
                              league_id=league_id, player_id=self.view_player)
                ...
                self.plot_try(self.plot_boards, #exception=None,
                              league_id=league_id, player_id=self.view_player, tab=league_tab)
    ...
    # plot viewer and badge
    def plot_viewer(self, league_id=None):
    ...
    def plot_caption(self, league_id=None, title=None, tab=None):
        plot_key = self.closet.get_key('viewer_img', league_id=league_id, player_id=self.view_player)
        stored, ok = self.closet.get_items(plot_key)
